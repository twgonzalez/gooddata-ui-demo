...
const DashboardMain = ({ dimensionItem }) => {
    ...
    const onApplyDateFilter = (newDateFilterOption, newExcludeCurrentPeriod) => {
        ...
        if (
            (newDateFilterOption.granularity === GRAIN_DAY && newDateFilterOption.from > -32) ||
            (newDateFilterOption.granularity === GRAIN_MONTH && newDateFilterOption.from > -2)
        ) {
            newChartDateGrain = Ldm.DateDatasets.Date.Date.MmDdYyyy;
        } else if (
            (newDateFilterOption.granularity === GRAIN_DAY && newDateFilterOption.from > -180) ||
            (newDateFilterOption.granularity === GRAIN_MONTH && newDateFilterOption.from > -7) ||
            (newDateFilterOption.granularity === GRAIN_QUARTER && newDateFilterOption.from > -3)
        ) {
